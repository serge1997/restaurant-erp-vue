<template>
    ...
    >
        ...
        <template #footer>
            <div v-if="!disablePaymentMethodBtn" class="d-flex flex-column">
                <Divider align="left">
                    <small class="c-gray-primary s-sm title3">forma de Pagamento</small>
                </Divider>
                <div class="pay-methods mb-2">
                    <Button 
                        v-for="method in paymentMethods"
                        :label="method.label"
                        :icon="`${method.icon} s-sm`"
                        :isDisable="disablePaymentMethodBtn"
                        iconPosition="left"
                        class="pm-btn text-capitalize"
                        :class="getSelectedPaymentMethodClass(method.value)"
                        @click="selectPaymentMethod(method.value)"
                    />
                </div>
                <div class="btn-button p-1 px-2">
                    <Button 
                        class="btn-green-primary w-100 d-flex justify-content-center m-auto fw-bold"
                        :label="`Fechar pedio . R$ ${getOrderTotal}`"
                        iconPosition="left"
                        icon="pi pi-check s-sm"
                        :isDisable="disableCloseOrderBtn"
                        @click="confirmOrderPayment"
                    />
                </div>
            </div>
            <div v-else class="d-flex flex-column border-top py-2">
                <div class="tag-greenlight s-sm text-center w-100">
                    ✓ Pedido fechado . {{ order.payment_method.label }}
                </div>
            </div>
        </template>
    </FormTemplate>
